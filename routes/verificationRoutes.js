...
const Verification = require("../models/Verification"); // your schema
...
router.post("/submit", upload.single("document"), async (req, res) => {
  try {
    ...
    const blob = await put(`verification/${Date.now()}-${req.file.originalname}`, req.file.buffer, {
      access: "public",
    ...
    res.json({ success: true, message: "Document uploaded successfully", data: verification });
  } catch (error) {
...
//   try {
//     console.log('Request received:', { body: req.body, file: req.file, user: req.user });
...
//       return res.status(400).json({ error: 'Valid document type is required (id, passport, or license)' });
//     }
//     if (!req.file) {
